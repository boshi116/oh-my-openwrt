...
function index()
    ...
    page = entry({"admin", "stuart", "fileassistant"}, template("fileassistant"), _("文件助手"), 2)
    page.i18n = "base"
    page.dependent = true

    page = entry({"admin", "stuart", "fileassistant_list"}, call("fileassistant_list"), nil)
    page.leaf = true

    page = entry({"admin", "stuart", "fileassistant_open"}, call("fileassistant_open"), nil)
    page.leaf = true

    page = entry({"admin", "stuart", "fileassistant_delete"}, call("fileassistant_delete"), nil)
    page.leaf = true

    page = entry({"admin", "stuart", "fileassistant_rename"}, call("fileassistant_rename"), nil)
    page.leaf = true
...
function fileassistant_install()
    ...
    elseif ext == "ipk" then
        ...
        os.execute('/bin/opkg --force-removal-of-dependent-packages --force-overwrite install"'..filepath..'"')
        success = true
    else
        success = false
    end
